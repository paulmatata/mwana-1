...
<div class="container">
    <div class="wizard-wrap">
        ...
        <h1>Step 1 of 4: Select the school</h1>
        <p class="sub">Choose your child's school from the list below.</p>
        ...
        @if ($schools->isEmpty())
            <p class="empty-note">No schools are registered on Mwana yet.</p>
        @else
            <form method="POST" action="{{ route('parent.first-login.school') }}">
                @csrf
                <label for="school_id">School</label>
                <select id="school_id" name="school_id" required>
                    <option value="">— Select a school —</option>
                    @foreach ($schools as $school)
                        <option value="{{ $school->id }}">{{ $school->name }}@if($school->county) &mdash; {{ $school->county }}@endif</option>
                    @endforeach
                </select>
